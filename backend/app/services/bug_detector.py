...
class BugDetector:
    ...
    def from_pytest_output(self, stdout: str, stderr: str) -> list[BugSignal]:
        ...
        if lint_lines:
            details = "\n".join(
                [f"{f}:{ln}:{c} {code} {msg}" for f, ln, c, code, msg in lint_lines[:10]]
            )
            signals.append(
                BugSignal(
                    kind="lint",
        ...
        if mypy:
            errs = [(f, ln, m) for f, ln, t, m in mypy if t == "error"]
            if errs:
                signals.append(
                    BugSignal(
                        kind="type_error",
                        summary=f"{len(errs)} mypy error(s)",
                        details="\n".join([f"{f}:{ln}: {m}" for f, ln, m in errs[:10]]),
                        file_hint=errs[0][0],
                        severity="error",
                    )
                )
        ...
        if ts_errs:
            signals.append(
                BugSignal(
                    ...
                    details="\n".join([f"{f}:{ln}: {m}" for f, ln, m in ts_errs[:10]]),
        ...
        if compile_errs:
            signals.append(
                BugSignal(
                    ...
                    details="\n".join([f"{f}:{ln}: {m}" for f, ln, m in compile_errs[:10]]),
    ...
    def from_cargo_output(self, stdout: str, stderr: str) -> list[BugSignal]:
        ...
        if errors:
            signals.append(
                BugSignal(
                    kind="syntax",
                    summary=f"{len(errors)} Rust compile error(s)",
                    details="\n".join(errors[:5]),
                    severity="error",
                )
            )
    ...
    def from_output(self, stdout: str, stderr: str, language: str | None = None) -> list[BugSignal]:
        ...
        if (
            "jest" in text.lower()
            or "vitest" in text.lower()
            or "mocha" in text.lower()
            or ".tsx" in text
            or ".ts:" in text
        ):
